    ...
    <form method="POST" action="{{ route('admin.magaza.update', $magaza) }}" class="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="platform" value="{{ old('platform', $magaza->platform) }}" class="w-full px-3 py-2 border rounded" placeholder="Trendyol, Hepsiburada, N11...">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-1">API Anahtarı</label>
                <input type="text" name="api_anahtari" value="{{ old('api_anahtari', $magaza->api_anahtari) }}" class="w-full px-3 py-2 border rounded">
            </div>
        </div>
