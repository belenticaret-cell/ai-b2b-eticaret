...
@section('title', 'E-Ticaret Site Ayarları')
@section('page-title', 'E-Ticaret Site Ayarları')
        ...
        <div class="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $stats['toplam_urun'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $stats['aktif_urun'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $stats['kategori_sayisi'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $stats['satisa_acik_kategori'] }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                ...
                <form action="{{ route('admin.site-ayar.update') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="site_adi" 
                                   value="{{ $ayarlar['site_adi'] ?? 'AI B2B E-Ticaret' }}"
                        ...
                        <div>
                            ...
                            <select name="site_aktif" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <option value="0" {{ ($ayarlar['site_aktif'] ?? '0') === '0' ? 'selected' : '' }}>🔴 Pasif</option>
                                <option value="1" {{ ($ayarlar['site_aktif'] ?? '0') === '1' ? 'selected' : '' }}>🟢 Aktif</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="site_aciklama" rows="3" 
                                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                  placeholder="Sitenizin kısa açıklaması...">{{ $ayarlar['site_aciklama'] ?? '' }}</textarea>
                    </div>
                            ...
                            @foreach($kategoriler as $kategori)
                                <label class="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                                    <input type="checkbox" name="satisa_acik_kategoriler[]" 
                                           value="{{ $kategori->id }}"
                                           {{ in_array($kategori->id, $secilenKategoriler) ? 'checked' : '' }}
                                           class="text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                                    <div class="ml-3 flex-1">
                                        <div class="font-medium text-gray-900">{{ $kategori->ad }}</div>
                                        <div class="text-sm text-gray-500">{{ $kategori->urunler_count ?? 0 }} ürün</div>
                                    </div>
                                </label>
                            @endforeach
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="color" name="site_renk_ana" 
                               value="{{ $ayarlar['site_renk_ana'] ?? '#3B82F6' }}"
                    ...
                    <div>
                        ...
                        <input type="tel" name="iletisim_telefon" 
                               value="{{ $ayarlar['iletisim_telefon'] ?? '' }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <span class="text-sm font-bold text-blue-600">{{ $stats['toplam_urun'] }}</span>
                    </div>
                    
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <span class="text-sm font-medium text-gray-700">Satışa Açık</span>
                        <span class="text-sm font-bold text-green-600">{{ $stats['aktif_urun'] }}</span>
                    </div>
...
function siteAyarApp() {
    return {
        siteAktif: {{ ($ayarlar['site_aktif'] ?? '0') === '1' ? 'true' : 'false' }},
        showToast: false,
        toastMessage: '',
        toastType: 'success',
        
        toggleSite() {
            fetch('{{ route('admin.site-ayar.toggle') }}', {
                method: 'POST',
                headers: {
